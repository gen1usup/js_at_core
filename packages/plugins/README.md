...
Lifecycle hook manager plus diagnostics JSON plugin.
...
```ts
import { PluginManager } from '@automation-platform/plugins';

const manager = new PluginManager(logger);
manager.register(new DiagnosticsReportPlugin({ outputDir: 'artifacts/plugins' }));
```
...
covered through core showcase; no package-local plugin tests yet

## Limitations

Only DiagnosticsReportPlugin is implemented; other plugin contracts are placeholders.
